...
def make_dataset(samples, predicates, joins, labels, max_num_joins, max_num_predicates):
    ...
    for sample in samples:
        ...
        # Use explicit length-based masking instead of feature mean
        sample_mask = np.zeros((max_num_joins + 1, 1), dtype=np.float32)
...
def get_train_datasets(num_queries, num_materialized_samples, queries_file="data/train", min_max_file="data/column_min_max_vals.csv"):
    dicts, column_min_max_vals, min_val, max_val, labels_train, labels_test, max_num_joins, max_num_predicates, train_data, test_data = load_and_encode_train_data(
        num_queries, num_materialized_samples, file_name_queries=queries_file, file_name_column_min_max_vals=min_max_file)
